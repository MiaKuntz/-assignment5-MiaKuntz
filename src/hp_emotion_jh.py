...
# Description: This script classifies the dialog in all CSV files in the input directory using the pretrained model from huggingface model hub (https://huggingface.co/j-hartmann/emotion-english-distilroberta-base) and plots a histogram for each CSV file.
...
def clf_data(dialog_df):
    ...
    # initializing pipeline for text classification with pretrained model from huggingface model hub (https://huggingface.co/j-hartmann/emotion-english-distilroberta-base)
...
def main():
    ...
    for file_name in os.listdir(input_dir):
        ...
        if file_name.endswith(".csv") and file_name != "movies.csv":
            ...
            out_file = os.path.join(output_dir, f"{file_name.split('.')[0]}_emotion_dialog_table_jh.csv")
            model_file = os.path.join(models_dir, f"{file_name.split('.')[0]}_emotion_dialog_hist_jh.png")
